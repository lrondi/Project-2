...
The first part consisted on creating the interactive map in which every species' location in the database can be visualized as markers (one species at a time), and at the same time be able to see the location of every species that share a given taxonomic rank with the selected species.
